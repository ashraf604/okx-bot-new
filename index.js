// OKX Portfolio Bot with PnL, Capital Setting, Egypt TZ, Live Trade Notifications, compatible with BotFather commands
...
bot.command("monitor", async ctx => {
    ...
    if (!global.monitoring) {
        ...
        await ctx.reply("✅ تم تشغيل المراقبة التلقائية للصفقات.");
    } else {
        await ctx.reply("ℹ️ المراقبة التلقائية تعمل بالفعل.");
    }
});
...
bot.command("stop_monitor", async ctx => {
    ...
    if (global.monitoring) {
        ...
        await ctx.reply("🛑 تم إيقاف المراقبة التلقائية للصفقات.");
    } else {
        await ctx.reply("ℹ️ المراقبة التلقائية ليست مفعلة حاليًا.");
    }
});
