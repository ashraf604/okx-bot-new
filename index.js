...
// تمت إضافة زر "🧮 حاسبة الربح والخسارة" إلى لوحة المفاتيح الرئيسية
...
// --- START: الإضافة الجديدة ---
// تمت إضافة معالج الأمر الجديد pnl
bot.command("pnl", async (ctx) => {
    if (ctx.from.id !== AUTHORIZED_USER_ID) return;

    // استخلاص الأرقام من الرسالة بعد الأمر
...
bot.on("message:text", async (ctx) => {
    ...
    switch (text) {
        ...
        // --- START: الإضافة الجديدة ---
        // تمت إضافة case للزر الجديد ليعرض التعليمات
...
if (process.env.NODE_ENV === "production") {
    ...
    app.listen(PORT, () => {
        ...
        bot.api.setWebhook(`https://YOUR_WEBHOOK_URL/${process.env.TELEGRAM_BOT_TOKEN}`);
        alertsCheckInterval = setInterval(checkAlerts, 60000); // 1 minute
        dailyJobsInterval = setInterval(runDailyJobs, 3600000 * 4); // 4 hours
    });
} else {
    bot.start({
