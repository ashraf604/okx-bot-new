// =================================================================
// OKX Advanced Analytics Bot - v58 (Final Stability & Feature Patch)
// =================================================================
// This version implements all critical fixes from the analysis report,
// including missing functions and a robust retry mechanism.
// =================================================================
...
let lastApiCall = 0;
const API_RATE_LIMIT = 200; // 5 requests per second max
...
bot.on("callback_query:data", async (ctx) => {
    try {
        ...
        if (data.startsWith("publish_")) {
            const publicMessage = ctx.callbackQuery.message.text.match(/🔔 \*\*توصية:[\s\S]*/);
            if (publicMessage) {
                try {
                    await bot.api.sendMessage(process.env.TARGET_CHANNEL_ID, publicMessage[0], { parse_mode: "Markdown" });
                 ...
                 await ctx.editMessageText("❌ خطأ: لم يتم العثور على نص التوصية للنشر.");
            }
            return;
        }
