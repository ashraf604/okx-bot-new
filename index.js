// =================================================================
// OKX Advanced Analytics Bot - v51 (Final Professional Version)
// =================================================================
// This version perfects the trade notification flow, providing a detailed
// private analysis followed by a strategic public recommendation.
// =================================================================
...
async function formatPortfolioMsg(assets, total, capital) {
    ...
    const positions = await loadPositions();
    let dailyPnlText = "   ▫️ *أداء (24س):* `لا توجد بيانات كافية`\n";
    if (history.length > 0) {
        ...
        if (previousDayRecord && typeof previousDayRecord.total === 'number') {
            ...
            dailyPnlText = `   ▫️ *أداء (24س):* ${dailyPnlEmoji} \`${dailyPnlSign}${(dailyPnl || 0).toFixed(2)}\` (\`${dailyPnlSign}${(dailyPnlPercent || 0).toFixed(2)}%\`)\n`;
        }
    }
    ...
    assets.forEach((a, index) => {
        ...
        if (a.asset === "USDT") {
            ...
            msg += `├─ *تغير (24س):* ${changeEmoji} \`${changeSign}${(change24hPercent || 0).toFixed(2)}%\`\n`;
            ...
            if (position && position.avgBuyPrice > 0) {
                ...
                msg += `╰─ *متوسط الشراء:* \`غير مسجل (سيتم تسجيله تلقائيًا مع أول صفقة شراء)\``;
            }
        }
...
async function monitorBalanceChanges() {
    try {
        ...
        for (const asset of allAssets) {
            ...
            let tradeType = "";
            if (difference > 0) { tradeType = "شراء 🟢"; } 
            else { tradeType = (currAmount * price < 1) ? "إغلاق مركز 🔴" : "بيع جزئي 🟠"; }
            ...
            const privateTradeAnalysisText = `🔔 **تحليل حركة تداول**\n` +
                ...
                `━━━━━━━━━━━━━━━━━━━━\n` +
                `📊 **التأثير على المحفظة:**\n` +
                `   - *حجم الصفقة من المحفظة:* \`${entryOfPortfolio.toFixed(2)}%\`\n` +
                `   - *الوزن الجديد للعملة:* \`${portfolioPercentage.toFixed(2)}%\`\n` +
                `   - *الرصيد النقدي الجديد:* \`$${newCashValue.toFixed(2)}\`\n` +
                `   - *نسبة الكاش الجديدة:* \`${newCashPercentage.toFixed(2)}%\`\n` +
                `━━━━━━━━━━━━━━━━━━━━\n` +
...
bot.on("callback_query:data", async (ctx) => {
    ...
    if (data.startsWith("chart_")) {
        ...
        if (period === '24h') { 
history = await loadHourlyHistory(); periodLabel = "آخر 24 ساعة"; periodData = history.slice(-24); }
        ...
        if (!periodData || periodData.length < 2) { await ctx.editMessageText("ℹ️ لا توجد بيانات كافية لإنشاء تقرير لهذه الفترة.");
return; }
...
try { await ctx.replyWithPhoto(chartUrl, { caption: caption, parse_mode: "Markdown" }); await ctx.deleteMessage(); } catch(e) { console.error("Failed to send chart:", e);
...
return;
    ...
    switch (data) {
        case "view_positions":
            const positions = await loadPositions();
if (Object.keys(positions).length === 0) { await ctx.editMessageText("ℹ️ لا توجد مراكز مفتوحة قيد التتبع حاليًا.", { reply_markup: new InlineKeyboard().text("🔙 العودة إلى الإعدادات", "back_to_settings") });
} else {
...
const totalAmountText = pos && pos.totalAmountBought ? pos.totalAmountBought.toFixed(6) : 'غير متاح';
                    const openDateText = pos && pos.openDate ?
new Date(pos.openDate).toLocaleDateString('en-GB') : 'غير متاح';
...
case "manage_movement_alerts": await sendMovementAlertsMenu(ctx); break;
        case "set_global_alert": waitingState = 'set_global_alert_state';
await ctx.editMessageText("✍️ يرجى إرسال النسبة المئوية العامة الجديدة لتنبيهات الحركة (مثال: `5` لـ 5%)."); break;
        case "set_coin_alert": waitingState = 'set_coin_alert_state';
...
if (Object.keys(alertSettings.overrides).length === 0) { msg_alerts += "لا توجد نسب مخصصة حاليًا."
} else { for (const coin in alertSettings.overrides) { msg_alerts += `- *${coin}:* \`${alertSettings.overrides[coin]}%\`\n`;
} }
...
break;
        case "set_capital": waitingState = 'set_capital'; await ctx.editMessageText("💰 يرجى إرسال المبلغ الجديد لرأس المال (رقم فقط).", { reply_markup: undefined });
break;
        case "delete_alert":
            const alerts = await loadAlerts();
if (alerts.length === 0) {
                await ctx.editMessageText("ℹ️ لا توجد تنبيهات سعر محدد مسجلة حاليًا.", { reply_markup: new InlineKeyboard().text("🔙 العودة إلى الإعدادات", "back_to_settings") });
} else {
...
msg += "\n*يرجى إرسال رقم التنبيه الذي تود حذفه.*";
            ...
            break;
case "toggle_summary": case "toggle_autopost": case "toggle_debug": { let settings = await loadSettings(); if (data === 'toggle_summary') settings.dailySummary = !settings.dailySummary;
else if (data === 'toggle_autopost') settings.autoPostToChannel = !settings.autoPostToChannel; else if (data === 'toggle_debug') settings.debugMode = !settings.debugMode; await saveSettings(settings); await sendSettingsMenu(ctx);
} break;
        case "delete_all_data": waitingState = 'confirm_delete_all'; await ctx.editMessageText("⚠️ *تحذير: هذا الإجراء لا يمكن التراجع عنه!* ⚠️\n\nسيتم حذف جميع بياناتك المخزنة، بما في ذلك رأس المال، المراكز، سجل الأداء، وجميع الإعدادات.\n\n*للمتابعة، أرسل كلمة: `تأكيد الحذف`*", { parse_mode: "Markdown", reply_markup: undefined });
setTimeout(() => { if (waitingState === 'confirm_delete_all') waitingState = null; }, 30000); break;
    }
});
...
bot.on("message:text", async (ctx) => {
    ...
    switch(text) {
        case "📊 عرض المحفظة":
            ...
            if (!pricesPortfolio) { return await ctx.reply("❌ 
عذرًا، فشل في جلب أسعار السوق. يرجى المحاولة مرة أخرى."); }
...
waitingState = null;
        switch (state) {
            case 'set_capital': const amount = parseFloat(text);
if (!isNaN(amount) && amount >= 0) { await saveCapital(amount); await ctx.reply(`✅ *تم تحديث رأس المال بنجاح.*\n\n💰 **المبلغ الجديد:** \`$${(amount || 0).toFixed(2)}\``, {parse_mode: "Markdown"});
} else { await ctx.reply("❌ مبلغ غير صالح. يرجى إرسال رقم فقط."); } return;
            case 'set_global_alert_state': const percent = parseFloat(text);
if (isNaN(percent) || percent <= 0) { return await ctx.reply("❌ *خطأ:* النسبة يجب أن تكون رقمًا موجبًا.");
} let alertSettingsGlobal = await loadAlertSettings(); alertSettingsGlobal.global = percent; await saveAlertSettings(alertSettingsGlobal);
await ctx.reply(`✅ تم تحديث النسبة العامة لتنبيهات الحركة إلى \`${percent}%\`.`); return;
            case 'set_coin_alert_state': const parts_coin_alert = text.split(/\s+/);
if (parts_coin_alert.length !== 2) { return await ctx.reply("❌ *صيغة غير صحيحة*. يرجى إرسال رمز العملة ثم النسبة.");
} const [symbol_coin_alert, percentStr_coin_alert] = parts_coin_alert; const coinPercent = parseFloat(percentStr_coin_alert);
if (isNaN(coinPercent) || coinPercent < 0) { return await ctx.reply("❌ *خطأ:* النسبة يجب أن تكون رقمًا.");
} let alertSettingsCoin = await loadAlertSettings(); if (coinPercent === 0) { delete alertSettingsCoin.overrides[symbol_coin_alert.toUpperCase()];
await ctx.reply(`✅ تم حذف الإعداد المخصص لـ *${symbol_coin_alert.toUpperCase()}* وستتبع الآن النسبة العامة.`); } else { alertSettingsCoin.overrides[symbol_coin_alert.toUpperCase()] = coinPercent;
await ctx.reply(`✅ تم تحديث النسبة المخصصة لـ *${symbol_coin_alert.toUpperCase()}* إلى \`${coinPercent}%\`.`); } await saveAlertSettings(alertSettingsCoin); return;
case 'coin_info':
...
await saveAlerts(currentAlerts);
                ...
                return;
case 'confirm_delete_all': if (text === 'تأكيد الحذف') { await getCollection("configs").deleteMany({});
await ctx.reply("✅ تم حذف جميع بياناتك المخزنة بنجاح. يمكنك البدء من جديد.");
} else { await ctx.reply("❌ تم إلغاء عملية الحذف."); } return;
        }
    }
});
