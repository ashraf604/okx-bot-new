// =================================================================
// OKX Advanced Analytics Bot - v29 (Advanced Performance Dashboard)
// =================================================================
// هذا الإصدار يضيف لوحة تحليل أداء متكاملة مع إحصائيات مفصلة.
// =================================================================
...
function createChartUrl(history, days) {
    const periodHistory = history.slice(-days);
    if (periodHistory.length < 2) return null;
    const labels = periodHistory.map(h => h.date.slice(5));
    const data = periodHistory.map(h => h.total.toFixed(2));
    const chartConfig = {
        ...
        options: { title: { display: true, text: `أداء المحفظة - آخر ${days} يومًا` } }
    };
    return `https://quickchart.io/chart?c=${encodeURIComponent(JSON.stringify(chartConfig))}&backgroundColor=white`;
}
function calculatePerformanceStats(history, days) {
    const periodHistory = history.slice(-days);
    if (periodHistory.length < 2) return null;

    const values = periodHistory.map(h => h.total);
...
bot.on("callback_query:data", async (ctx) => {
    ...
    if (data.startsWith("chart_")) {
        ...
        const stats = calculatePerformanceStats(history, days);
        if (!stats) {
            await ctx.editMessageText("ℹ️ لا توجد بيانات كافية لهذه الفترة.");
            return;
        }
        const chartUrl = createChartUrl(history, days);
        const pnlEmoji = stats.pnl >= 0 ? '🟢' : '🔴';
        const caption = `📊 *أداء المحفظة - آخر ${days} يومًا*\n\n` +
        ...
        case "📈 أداء المحفظة":
            ...
            if (history.length < 2) {
                ...
                const keyboard = new InlineKeyboard()
                    .text("آخر 7 أيام", "chart_7")
                    .text("آخر 30 يومًا", "chart_30");
