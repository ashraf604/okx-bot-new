...
bot.callbackQuery("delete_alert", async (ctx) => {
    ...
    switch (text) {
        ...
        case "📈 أداء المحفظة":
            ...
            if (chartUrl) {
                ...
                const summary = `📈 *تغير اليوم الأخير:* ${diff >= 0 ? '🟢' : '🔴'} $${diff.toFixed(2)} (${percent.toFixed(2)}%)`;
...
app.listen(PORT, async () => {
    ...
    if (!dailyJobsInterval) {
        dailyJobsInterval = setInterval(runDailyJobs, 60000);
        console.log("✅ Daily jobs scheduler started.");
    }
    try {
