// OKX Portfolio Bot with PnL, Capital Setting, Clean Filtering, Egypt TZ, and Trade Monitoring
...
const app = express(); const bot = new Bot(process.env.TELEGRAM_BOT_TOKEN); const PORT = process.env.PORT || 3000; const AUTHORIZED_USER_ID = parseInt(process.env.AUTHORIZED_USER_ID); const API_BASE_URL = "https://www.okx.com"; const CAPITAL_FILE = "capital.json"; let monitoring = false; let previousTrades = [];
...
function getHeaders(method, path, body = "") { const timestamp = new Date().toISOString(); const prehash = timestamp + method.toUpperCase() + path + body; const crypto = require("crypto"); const sign = crypto.createHmac("sha256", process.env.OKX_API_SECRET_KEY).update(prehash).digest("base64"); return { "OK-ACCESS-KEY": process.env.OKX_API_KEY, "OK-ACCESS-SIGN": sign, "OK-ACCESS-TIMESTAMP": timestamp, "OK-ACCESS-PASSPHRASE": process.env.OKX_API_PASSPHRASE, "Content-Type": "application/json", }; }
...
async function checkNewTrades() { if (!monitoring) return; try { const res = await fetch(${API_BASE_URL}/api/v5/trade/fills?instType=SPOT&limit=20, { headers: getHeaders("GET", "/api/v5/trade/fills?instType=SPOT&limit=20") }); const json = await res.json(); if (json.code !== "0") return;

const newTrades = json.data.filter(trade => !previousTrades.includes(trade.tradeId));
    if (newTrades.length > 0) {
        previousTrades.push(...newTrades.map(t => t.tradeId));
        let summary = {};
        newTrades.forEach(t => {
            if (!summary[t.instId]) summary[t.instId] = { qty: 0, total: 0, count: 0 };
            summary[t.instId].qty += parseFloat(t.sz);
            summary[t.instId].total += parseFloat(t.fillPx) * parseFloat(t.sz);
            summary[t.instId].count++;
        });

        let msg = `🚨 *تم رصد صفقات جديدة:*\n\n`;
        for (const instId in summary) {
            const avgPrice = summary[instId].total / summary[instId].qty;
            msg += `💎 *${instId}*\n`;
            msg += `  الكمية: ${summary[instId].qty}\n`;
            msg += `  السعر المتوسط: $${avgPrice.toFixed(4)}\n`;
            msg += `  عدد الصفقات: ${summary[instId].count}\n\n`;
        }
...
setInterval(checkNewTrades, 2 * 60 * 1000);

bot.command("start", async ctx => { if (ctx.from.id !== AUTHORIZED_USER_ID) return; const keyboard = new InlineKeyboard() .text("📊 عرض المحفظة", "refresh") .text(monitoring ? "🛑 إيقاف المراقبة" : "✅ تفعيل المراقبة", "togglemonitor") .text("⚙️ تعيين رأس المال", "setcapital"); await ctx.reply("🤖 أهلاً بك في بوت مراقبة محفظة OKX\n\n- اختر الإجراء المطلوب.", { parse_mode: "Markdown", reply_markup: keyboard }); });
...
bot.callbackQuery("togglemonitor", async ctx => { monitoring = !monitoring; await ctx.answerCallbackQuery(monitoring ? "✅ تم تشغيل المراقبة." : "🛑 تم إيقاف المراقبة."); await ctx.reply(monitoring ? "✅ تم تشغيل مراقبة الصفقات الجديدة تلقائياً." : "🛑 تم إيقاف مراقبة الصفقات الجديدة.", { parse_mode: "Markdown" }); });
