// OKX Portfolio Bot with PnL, Capital Setting, Egypt TZ, Live Trade Notifications, and Working Capital Set

const express = require("express"); const { Bot, InlineKeyboard, webhookCallback, session } = require("grammy"); const fetch = require("node-fetch"); const crypto = require("crypto"); const fs = require("fs"); require("dotenv").config();

const app = express(); const bot = new Bot(process.env.TELEGRAM_BOT_TOKEN); const PORT = process.env.PORT || 3000; const AUTHORIZED_USER_ID = parseInt(process.env.AUTHORIZED_USER_ID); const API_BASE_URL = "https://www.okx.com"; const CAPITAL_FILE = "capital.json"; let lastTrades = {}; // Track last trades to avoid duplication

bot.use(session({ initial: () => ({ waitingForCapital: false }) }));
...
bot.callbackQuery("setcapital", async ctx => { await ctx.answerCallbackQuery(); ctx.session.waitingForCapital = true; await ctx.reply("💼 أرسل المبلغ الآن لتعيين رأس المال بالدولار، مثال: 5000"); });

bot.callbackQuery("monitor", async ctx => { await ctx.answerCallbackQuery("✅ تم تشغيل مراقبة الصفقات."); if (!global.monitoring) { global.monitoring = setInterval(checkNewTrades, 60000); // Every 1 minute } });

bot.on("message:text", async ctx => { if (ctx.from.id !== AUTHORIZED_USER_ID) return; if (ctx.session.waitingForCapital) { const amount = parseFloat(ctx.message.text.trim()); if (!isNaN(amount) && amount > 0) { saveCapital(amount); await ctx.reply(✅ تم تعيين رأس المال إلى: $${amount.toFixed(2)}); } else { await ctx.reply("❌ المبلغ غير صالح."); } ctx.session.waitingForCapital = false; } });
