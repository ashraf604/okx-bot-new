...
async function runHourlyJobs() { try { const prices = await getMarketPrices(); if (!prices) return; const { total, error } = await getPortfolio(prices); if (error) return; const history = await loadHourlyHistory(); const now = new Date(); const hourLabel = now.toISOString().slice(0, 13); const existingIndex = history.findIndex(h => h.label === hourLabel); if (existingIndex > -1) { history[existingIndex].total = total; } else { history.push({ label: hourLabel, total: total }); } if (history.length > 72) { history.splice(0, history.length - 72); } await saveHourlyHistory(history); await sendDebugMessage(`تم حفظ السجل الساعي: ${hourLabel} - $${total.toFixed(2)}`); } catch (e) { console.error("خطأ في المهام الساعية:", e); } }
...
bot.on("callback_query:data", async (ctx) => {
    try {
        ...
        if (data.startsWith("publish_")) {
            // FIX v59: Corrected the replacement string to match the actual prompt message.
...
async function startBot() {
    try {
        ...
        setInterval(runHourlyJobs, 3600000);       // Run hourly jobs (performance snapshot) every hour
        setInterval(runDailyJobs, 86400000);       // Run daily jobs (e.g. cleanup) every 24 hours - Note: Daily report is snapshot-based, while main view is now real-time 24h.

        if (process.env.NODE_ENV === "production") {
            app.use(express.json());
            app.use(webhookCallback(bot, "express"));
            app.listen(PORT, () => { console.log(`Bot v59 (Consistent Daily P&L Logic) is listening on port ${PORT}`); });
        } else {
            await bot.start();
            console.log("Bot v59 (Consistent Daily P&L Logic) started with polling.");
        }
    } catch (e) {
