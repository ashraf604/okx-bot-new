// =================================================================
// OKX Advanced Analytics Bot - v56 (Critical Logic Overhaul)
// =================================================================
// This version fixes two critical logic flaws:
// 1. A more robust check for closed positions (value < $1).
// 2. Removal of the premature 'return' in the trade monitoring loop,
//    allowing the bot to process multiple trades in a single cycle.
// =================================================================
...
async function monitorBalanceChanges() {
    try {
        ...
        if (!prices) {
            await sendDebugMessage("فشل جلب أسعار السوق (استجابة غير صالحة من OKX)، سيتم إعادة المحاولة في الدورة التالية.");
            return;
        }
        ...
        for (const asset of allAssets) {
            ...
            if (type === 'شراء') {
                ...
                // We need to calculate portfolio percentage based on value BEFORE the trade
                ...
                callbackData = `publish_buy_${asset}`;
            } else { // Sell
                // FIX 1: More robust check for a closed position
            ...
            if (settings.autoPostToChannel && publicRecommendationText) {
             ...
             // FIX 2: Removed the premature 'return' statement
        }
        ...
        if (tradesDetected) {
            ...
            // Also update the state even if no trades, to keep totalValue fresh
            await saveBalanceState({ balances: currentBalance, totalValue: newTotalPortfolioValue });
        }
    } catch (e) {
...
const mainKeyboard = new Keyboard().text("📊 عرض المحفظة").text("📈 أداء المحفظة").row().text("ℹ️ معلومات عملة").text("🔔 ضبط تنبيه").row().text("🧮 حاسبة الربح والخسارة").row().text("⚙️ الإعدادات").resized();
...
bot.on("callback_query:data", async (ctx) => {
    try {
        ...
        if (data.startsWith("publish_")) {
            ...
            } catch (e) { console.error("Failed to post to channel:", e); await ctx.editMessageText("❌ فشل النشر في القناة. تأكد من أن البوت لديه صلاحيات النشر في القناة المستهدفة.", { reply_markup: undefined }); }
            return;
        }
        ...
        if (waitingState) {
            ...
            switch (state) {
                ...
                case 'coin_info':
                    ...
                    const prices = await getMarketPrices();
                    if (prices) { const { assets: userAssets } = await getPortfolio(prices); const coinSymbol = instId.split('-')[0]; const ownedAsset = userAssets.find(a => a.asset === coinSymbol); const positions = await loadPositions(); const assetPosition = positions[coinSymbol]; if (ownedAsset && assetPosition && assetPosition.avgBuyPrice) { const amount = ownedAsset.amount; const avgBuyPrice = assetPosition.avgBuyPrice; const totalCost = avgBuyPrice * amount; const totalPnl = (details.price * amount) - totalCost; const totalPnlPercent = totalCost > 0 ? (totalPnl / totalCost) * 100 : 0; const totalPnlEmoji = totalPnl >= 0 ? '🟢⬆️' : '🔴⬇️'; const totalPnlSign = totalPnl >= 0 ? '+' : ''; const dailyPnl = (details.price - details.open24h) * amount; const dailyPnlEmoji = dailyPnl >= 0 ? '🟢⬆️' : '🔴⬇️'; const dailyPnlSign = dailyPnl >= 0 ? '+' : ''; msg += `\n\n━━━━━━━━━━━━━━━━━━━━\n` + `📊 *تحليل مركزك في العملة:*\n` + `   ▫️ *الربح/الخسارة الإجمالي (غير محقق):* ${totalPnlEmoji} \`${totalPnlSign}${(totalPnl || 0).toFixed(2)}\` (\`${totalPnlSign}${(totalPnlPercent || 0).toFixed(2)}%\`)\n` + `   ▫️ *الربح/الخسارة (آخر 24س):* ${dailyPnlEmoji} \`${dailyPnlSign}${(dailyPnl || 0).toFixed(2)}\``; } }
                    await ctx.reply(msg, { parse_mode: "Markdown" });
                    return;
                case 'set_alert': const parts_alert = text.trim().split(/\s+/); if (parts_alert.length !== 3) { return await ctx.reply("❌ صيغة غير صحيحة. يرجى استخدام الصيغة: `SYMBOL > PRICE`"); } const [alertInstId, condition, priceStr] = parts_alert; if (condition !== '>' && condition !== '<') { return await ctx.reply("❌ الشرط غير صالح. استخدم `>` أو `<` فقط."); } const alertPrice = parseFloat(priceStr); if (isNaN(alertPrice) || alertPrice <= 0) { return await ctx.reply("❌ السعر غير صالح."); } const alertsList = await loadAlerts(); alertsList.push({ instId: alertInstId.toUpperCase(), condition: condition, price: alertPrice }); await saveAlerts(alertsList); await ctx.reply(`✅ تم ضبط التنبيه بنجاح:\nسيتم إعلامك إذا أصبح سعر *${alertInstId.toUpperCase()}* ${condition === '>' ? 'أعلى من' : 'أقل من'} *${alertPrice}*`, { parse_mode: "Markdown" }); return;
                case 'delete_alert_number': const alertIndex = parseInt(text) - 1; let currentAlerts = await loadAlerts(); if (isNaN(alertIndex) || alertIndex < 0 || alertIndex >= currentAlerts.length) { return await ctx.reply("❌ رقم غير صالح. يرجى الاختيار من القائمة."); } const removedAlert = currentAlerts.splice(alertIndex, 1)[0]; await saveAlerts(currentAlerts); await ctx.reply(`✅ تم حذف التنبيه بنجاح:\n\`${removedAlert.instId} ${removedAlert.condition} ${removedAlert.price}\``, { parse_mode: "Markdown" }); return;
...
async function startBot() {
    try {
        ...
        if (process.env.NODE_ENV === "production") {
            ...
            app.listen(PORT, () => { console.log(`بوت v56 (Critical Logic Overhaul) يستمع على المنفذ ${PORT}`); });
        } else {
            bot.start();
            console.log("Bot v56 (Critical Logic Overhaul) started with polling.");
        }
